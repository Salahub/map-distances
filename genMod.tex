...
\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{ccc}
      ...
      {\footnotesize (b) JAX and BSB correlations against} &
      {\footnotesize (c) JAX and BSB correlations against} \\
      &
      {\footnotesize simulated populations of 80} &
      {\footnotesize paired populations of 66 and 94} \\
    \end{tabular}
  \end{center}
  \caption{The pairwise units used to construct the correlation distribution and correlation test plots.}
  \label{fig:2by2}
\end{figure}
...
The bar plot in Figure \ref{fig:discrete}(b) places lines with lengths proportional to the number of observed populations with a given correlation at the corresponding point on the horizontal axis. Seven clear clusters are visible, and a handful of lines suggest an eighth if a larger sample were simulated. The first of these from the right is at 1, and is a single line rather than a collection of lines. The peaks of each of the following groups of lines correspond to the peaks in Figure \ref{fig:discrete}(a). This pattern can be explained by the features of the backcross being simulated and the equation for sample correlation.
...
\m{M}_X = \begin{bmatrix}
  ...
  1 & 0 \\
\end{bmatrix}.$$
Without loss of generality, take $f = 1$. Absent cross overs there are only two possible values for $\ve{z}$: $\tr{(1,1)}$ and $\tr{(2,2)}$. Both of these values of $\ve{z}$ are equally likely, as the inherited variants are independently donated by each parent. Regardless of which is inherited, $Z_{ij} = Z_{ik}$ for the $i^{\text{th}}$ offspring in a population of $n$. The correlation, given by
\begin{equation} \label{eq:sampleCorr}
Corr(Z_j, Z_k) = \frac{\sum_{i = 1}^n z_{ij} z_{ik} - n \bar{z}_{j} \bar{z}_{k}}{\sqrt{ \left ( \sum_{i = 1}^n z_{ij}^2 - n \bar{z}_{j}^2 \right ) \left ( \sum_{i = 1}^n z_{ik}^2 - n \bar{z}_{k}^2 \right )}}
\end{equation}
where $\bar{x}_l = \frac{1}{n} \sum_{i = 1}^n x_{il}$, is always 1 in this case.

Suppose one cross over occurs and is inherited by the $n^{\text{th}}$ offspring, however. Then there is one individual in the population with $\ve{z} = \tr{(1,2)}$ or $\ve{z} = \tr{(2,1)}$. Noting the symmetry of this problem, take the first and consider $Z_k$ as the \emph{reference marker}. That is, we will assume the individual with $\ve{z} = \tr{(1,2)}$ would have been $\ve{z} = \tr{(2,2)}$ without a cross over. Let the correlation is given by Equation \ref{eq:sampleCorr} give the correlation of this reference case. With this single cross over, any sum over $z_{ij}$ in this equation will change.

The sum $\sum_{i = 1}^n z_{ji}$ decreases by 1 as the $n^{text{th}}$ term goes from 2 to 1. Therefore the product $n \bar{z}_{j}$ will decrease by one. Similarly, the sum
$$\sum_{i = 1}^n z_{ij}^2$$
will decrease by 3. Finally, the cross sum
$$\sum_{i = 1}^n z_{ji} z_{ki},$$
will decrease by 2, as in the reference case the $n^{\text{th}}$ term is given by $2 \times 2$, but with a cross over the term decreases to $2 \times 1$. Together, these change Equation \ref{eq:sampleCorr} to
$$\frac{ \left ( \sum_{i = 1}^n z_{ij} z_{ik} - 2 \right ) - \left( n \bar{z}_{j} - 1 \right ) \bar{z}_{k}}{\sqrt{ \left ( \left [ \sum_{i = 1}^n z_{ij}^2 - 3 \right ] -  n \left [ \bar{z}_{j} - \frac{1}{n} \right ]^2 \right ) \left ( \sum_{i = 1}^n z_{ik}^2 - n \bar{z}_{k}^2 \right )}}$$

$$= \frac{ \left ( \sum_{i = 1}^n z_{ij} z_{ik} - n \bar{z}_{j}\bar{z}_{k} \right ) - \left ( 2 - \bar{z_k} \right )}{\sqrt{ \left ( \left [ \sum_{i = 1}^n z_{ij}^2 -  n\bar{z}^2_{j} \right ] - \left [ 3 - 2 \bar{z}_j + \frac{1}{n} \right ] \right ) \left ( \sum_{i = 1}^n z_{ik}^2 - n \bar{z}_{k}^2 \right )}},$$

$$= \frac{ \left ( \sum_{i = 1}^n z_{ij} z_{ik} - n \bar{z}_{j}\bar{z}_{k} \right ) - \left ( 2 - \bar{z_k} \right )}{\sqrt{ \left ( \left [ \sum_{i = 1}^n z_{ij}^2 -  n\bar{z}^2_{j} \right ] - \left [ 3 - 2 \bar{z}_k + \frac{1}{n} \right ] \right ) \left ( \sum_{i = 1}^n z_{ik}^2 - n \bar{z}_{k}^2 \right )}},$$
and so the correlation is impacted by the mean of the reference marker and the sample size for each cross over! As $\bar{z}_k \in [1, 2]$, this will always decrease the correlation. The decrease will change, however, with the reference mean $\bar{z}_k$.

This logic can be extended to multiple cross overs. Using the concept of a reference marker, the correlations were computed as above for the cases of zero, one, two, three, and four cross overs for a selection of seven mean additive scores. Figure \ref{fig:bardetail} displays these settings with coloured lines in the bar plot of Figure \ref{fig:discrete}(b).
\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{cc}
  ...
  \label{fig:bardetail}
\end{figure}
These theoretical lines match the values observed in the simulated distribution exactly. Interestingly, correlation seems dominantly impacted by the number of cross overs, the mean additive score of the reference marker changes the correlation by small amounts in comparison.
