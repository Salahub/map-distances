...
The data sets require further cleaning before being used, however. First, any markers which do not have a known position along a chromosome in cMs are removed for both data sets. Following that, any individual mice with incomplete data are dropped from the panel. For the JAX BSB data this leaves 94 mice annotated at 1496 markers while the UCLA BSB data has 66 mice annotated at 111 markers. The correlation matrices for these data sets are displayed in Figures \ref{fig:jaxbsb}(a) and \ref{fig:uclabsb}(a) respectively.
...
A likely culprit is the assumption of independent cross overs. \cite{bromanetal2002crossover} evaluated the pattern of cross overs in the \cite{roweetal1994jaxbsb} experiment, the basis of the JAX BSB data. Their results suggest that there is some \emph{interference} between cross overs, that they are not fully independent. In particular they found that cross overs rarely occur within 20 cM of each other, and that it is rare to observe more than 2 cross overs on the same chromosome in a single meiosis event. These empirical results will have little impact on the correlation between markers with short distances between them, as more than one cross over event is unlikely to occur between these. Markers separated by longer distances are impacted by this observed interference to a greater extent. Triple crosses in particular may be rarer than expected under the model of independence, and so these markers are more likely to be separated, resulting in a lower correlation than under independence.

It is important not to over-interpret this pattern, however. It is not repeated in Figure \ref{fig:uclabsb} and the quantile regions which are shaded have not been adjusted to account for the many multiple tests performed in each plot.

In an effort to gain deeper insight, the common markers measured between the UCLA BSB and JAX BSB cross were identified and the correlation matrices recomputed. The correlations for these common markers are displayed in Figure \ref{fig:bsbcommon}. Most chromosomes have only one marker measured in common between these experiments, but chromosomes 2, 4, and 18 are noteworthy among the autosomal chromosomes for having several markers measured in common.

\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{cc}
      \includegraphics[width = 0.400\textwidth]{./img/jaxbsb_common.png} &
      \includegraphics[width = 0.400\textwidth]{./img/uclabsb_common.png} \\
...
Consider the eight markers in common on chromosomes 2 and 4.
