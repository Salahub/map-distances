...
Starting with $\m{G}$, the whole genome of an individual organism, the \emph{select} step chooses which regions to measured. In human studies this is supported by SNP databases such as \cite{NCBIdbSNP} which document hundreds of millions of common SNPs in the human genome. Only a small proportion of these are estimated to occur in greater than 1\% of the population, leading \cite{koboldtetal2013next} to suggest that most of the common SNP variations have already been sequenced and catalogued. This gives roughly 15 million SNPs to chose from, with \cite{laframboise2009} reporting that modern SNP microarray platforms can simultaneously identify several million SNPs. The third generation of genome sequencing technologies allows for entire genomes to be sequenced, as noted in \cite{heatherchain2016sequencers, hasinetal2017multi}, but the challenges of the still young chemistry of genome sequencing and decades of work cataloguing SNPs and innovating SNP microarray methods . \cite{laframboise2009} outlines the chemistry of the dominant means of selection, \emph{SNP microarrays}. These microarrays use a   The  \cite{laframboise2009} details the importance of modern SNP microarrays in the selection step. Iterative improvements in computational methods have led to the identification of roughly one million SNPs which can be used to survey the whole genome due to \emph{linkage disequilibrium}, essentially the correlation between positions on the genome. \cite{hasinetal2017multi} expand this in light of modern \emph{next-generation sequencing} (NGS) methods.

\cite{hasinetal2017multi} also provide justification for the relationship between animal models and human research.
