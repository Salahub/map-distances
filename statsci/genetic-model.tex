...
%%                                          %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{A structural model for genome-wide association studies}
...
\address[A]{Chris Salahub is a PhD Candidate in Statistics, Statistics
  and Acturial Science, University of Waterloo, Waterloo,
...
\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{c}
      \includegraphics[scale = 0.3]{../img/bsbCorr2by2.png} \\
      {\footnotesize (a) Distribution of correlations} \\
      \\
      \includegraphics[scale = 0.3]{../img/bsbCorrTest2by2a.png} \\
      {\footnotesize (b) JAX and BSB correlations against} \\
      {\footnotesize simulated populations of 80} \\
      \\
      \includegraphics[scale = 0.3]{../img/bsbCorrTest2by2.png} \\
...
\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{c}
      \includegraphics[scale = 0.3]{../img/strngDen.png} \\
      {\footnotesize (a) KDE} \\
      \\
      \includegraphics[scale = 0.3]{../img/strngBarNoCol.png} \\
...
Suppose $m$ cross overs occur and are inherited by the last $m$ offspring. Then there are $m$ individuals in the population with $\ve{z} = \tr{(1,2)}$ or $\ve{z} = \tr{(2,1)}$. For simplicity, let all cross overs lead to $\ve{z} = \tr{(1,2)}$. Take $Z_k$ as the \emph{reference marker} and assume the individuals with $\ve{z} = \tr{(1,2)}$ would have been $\ve{z} = \tr{(2,2)}$ without a cross over. Let
\begin{equation*} \label{eq:refCorr}
Corr_{ref} := \frac{ s_{jk}^* - n \bar{z}_{j}^* \bar{z}_{k}^*}{\sqrt{ \left ( s_{jj}^* - n (\bar{z}_{j}^*)^2 \right ) \left ( s_{kk}^* - n (\bar{z}_{k}^*)^2 \right )}}
\end{equation*}
...
The sum $\sum_{i = 1}^n z_{ji}$ decreases by $m$ since each crossed over term goes from 2 to 1. Therefore, the product $n \bar{z}_{j}$ will also decrease by $m$. Similarly, the sum $\sum_{i = 1}^n z_{ij}^2$
will decrease by $3m$ as each crossed over term goes from 4 to 1. Finally, the sum $\sum_{i = 1}^n z_{ji} z_{ki},$ will decrease by $2m$, as in the reference case the crossed over terms are 4, but are now 2. Together, these change Equation \ref{eq:refCorr} to
\begin{equation*}
  \begin{split}\frac{ \left ( s^*_{jk} - 2m \right ) - \left( n \bar{z}_{j}^* - m \right ) \bar{z}_{k}^*}{\sqrt{ \left ( \left [ s_{jj}^* - 3m \right ] -  n \left [ \bar{z}_{j}^* - \frac{m}{n} \right ]^2 \right ) \left ( s_{kk}^* - n (\bar{z}_{k}^*)^2 \right )}} \\
    \\
    = \frac{ \left ( s_{jk}^* - n \bar{z}_j^* \bar{z}_k^* \right ) - m \left( 2 - \bar{z}_{k}^* \right )}{\sqrt{ \left ( \left [ s_{jj}^* - n (\bar{z}_j^*)^2 \right ] - m \left [ 3 - 2 \bar{z}_{j}^* + \frac{m}{n} \right ] \right ) \left ( s_{kk}^* - n (\bar{z}_{k}^*)^2 \right )}} \\
    \\
    = \frac{ \left ( s_{jk}^* - n \bar{z}_j^* \bar{z}_k^* \right ) - m \left( 2 - \bar{z}_{k}^* \right )}{\sqrt{ \left ( \left [ s_{jj}^* - n (\bar{z}_j^*)^2 \right ] - m \left [ 3 - 2 \bar{z}_{k}^* + \frac{m}{n} \right ] \right ) \left ( s_{kk}^* - n (\bar{z}_{k}^*)^2 \right )}}. \\
  \end{split}
\end{equation*}
Noting that $\bar{z}_j^* = \bar{z}_k^* := \bar{z}$ and $s_{jj}^* = s_{kk}^* = s_{jk}^* := s_{..}$ by construction, this can be further simplified to
\begin{equation} \label{eq:crossedCorr}
Corr(Z_j, Z_k) = \frac{ \left ( s_{..} - n \bar{z}^2 \right ) - m \left( 2 - \bar{z}^2 \right )}{\sqrt{ \left ( s_{..} - n \bar{z}^2 \right )^2 - m \left ( 3 - 2 \bar{z} + \frac{m}{n} \right )  \left ( s_{..} - n \bar{z}^2 \right ) }}. \\
\end{equation}
...
Using the above, the correlations were computed for the cases of zero, one, two, three, and four cross overs for a selection of seven mean additive scores. Figure \ref{fig:bardetail} displays these settings with coloured lines laid under the bar plot of Figure \ref{fig:discrete}(b).
\begin{figure}[htp]
  \begin{center}
    \begin{tabular}{c}
      \includegraphics[scale = 0.3]{../img/strngBar.png} \\
      {\footnotesize (a) Coloured by count of cross overs} \\
      \\
      \includegraphics[scale = 0.3]{../img/strngBarClose.png} \\
  ...
  \label{fig:bardetail}
\end{figure}
These theoretical lines match the values observed in the simulated distribution exactly. As can be deduced from the form of Equation \ref{eq:crossedCorr}, the correlation is dominantly impacted by the number of cross overs, the mean additive score of the reference marker changes the correlation by small amounts in comparison.
